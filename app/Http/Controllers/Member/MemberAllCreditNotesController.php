...
class MemberAllCreditNotesController extends MemberBaseController
{
    ...
    public function data(Request $request)
    {
        ...
        return DataTables::of($creditNotes)
            ...
            ->addColumn('action', function ($row) use($firstCreditNotes){
                ...
                if ($this->user->can('view_invoices')) {
                    $action .= '<li><a href="' . route("member.all-credit-notes.download", $row->id) . '"><i class="fa fa-download"></i> Download</a></li>';
                }

                if ($this->user->can('edit_invoices')) {
                    $action .= '<li><a href="' . route("member.all-credit-notes.edit", $row->id) . '"><i class="fa fa-pencil"></i> Edit</a></li>';
                }
    ...
    public function store(StoreCreditNotes $request)
    {
        ...
        //log search
        $this->logSearchEntry($creditNote->id, 'CreditNote ' . $creditNote->cn_number, 'member.all-credit-notes.show');
